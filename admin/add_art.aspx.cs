    ...
    public partial class add_art : AdminPageBase
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                CheckBoxListSite.DataSource = commonService.GetSiteList();
                CheckBoxListSite.DataValueField = "site_id";
                CheckBoxListSite.DataTextField = "site_name";
                CheckBoxListSite.DataBind();

                CheckBoxListSite.Items.FindByValue(Total.SiteId).Selected = true;
        ...
        private void getUpdateNews(string nid, bool isDeleted)
        {
            ...
            {
                CheckBoxListSite.Items.FindByValue(siteId).Selected = true;
            }
            if (!isRefArticle)
            {
        ...
        private void submit(object sender, System.EventArgs e, bool isStandard)
        {
            ...
            foreach (ListItem item in CheckBoxListSite.Items)
            {
                if (item.Selected)
                    siteIdList.Add(item.Value);
            }
